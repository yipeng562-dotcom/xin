...
public sealed class MainForm : Form
{
    ...
    public MainForm(int port = 8765, string? exportDirectory = null)
    {
        _port = port;
        _liveExporter = new LiveStateExporter("x32", port, exportDirectory);
        ...
        Text = "x32dbg Snapshot Assistant";
        Width = 1380;
        Height = 920;
    ...
    private Control BuildRightTabs()
    {
        ...
        _smartCopyButton.Text = "等待 x32dbg...";
    ...
    private void UpdateStatus()
    {
        if(!_server.IsListening)
            TryStartServer();
        var connected = _lastTcpAt.HasValue && DateTime.Now - _lastTcpAt.Value < TimeSpan.FromSeconds(60);
        if(connected)
        {
            _connectionStatus.Text = $"● x32dbg 已连接  127.0.0.1:{_port}";
            _connectionStatus.ForeColor = Color.LightGreen;
        }
        else if(_server.IsListening)
        {
            _connectionStatus.Text = $"● x32dbg 未连接  监听 {_port}";
    ...
    private void UpdateAutomaticSummary()
    {
        ...
        var browseAddress = browse is not null && browse.SelectedAddress != "00000000" ? browse.SelectedAddress : "-";
        ...
        _sessionStatus.Text = $"真实EIP：{cpuAddress}";
