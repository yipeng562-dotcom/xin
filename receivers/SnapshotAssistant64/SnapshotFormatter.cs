...
{
    private static string RecordedAt(long timestamp)
    {
        try { return DateTimeOffset.FromUnixTimeMilliseconds(timestamp).ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"); }
        catch(ArgumentOutOfRangeException) { return "时间戳异常"; }
    }
        ...
        SnapshotEvent? selection = null)
    {
        ...
        if(!HexUtil.HasAddress(browseAddress))
            browseAddress = snapshot.SelectedAddress;
        var hasBrowse = HexUtil.HasAddress(browseAddress);
        ...
        builder.AppendLine($"真实RIP：{snapshot.Eip}");
        if(snapshot.Origin == EventOrigin.Fallback)
            builder.AppendLine($"来源：回退日志补收（记录时间：{RecordedAt(snapshot.TimestampMs)}）");
        ...
        var eflags = Register(snapshot, "RFLAGS");
        builder.AppendLine($"RFLAGS={eflags}");
        ...
        foreach(var slot in snapshot.Stack.Take(stackLimit))
            builder.AppendLine($"RSP+{OffsetShort(slot.Offset)} [{slot.Address}] = {slot.Value}");
        if(snapshot.Stack.Count == 0)
            builder.AppendLine("暂无栈数据");

        builder.AppendLine().AppendLine("===== CPU附近汇编 =====").AppendLine();
        var cpuContext = snapshot.DisassemblyContext.Any(item => item.Address == snapshot.Eip)
            ? snapshot.DisassemblyContext : Array.Empty<InstructionInfo>();
        var assembly = fullAssembly ? cpuContext : SliceContext(cpuContext, snapshot.Eip, 12, 20);
    ...
    public static string FormatBrowse(SnapshotEvent? selection, SnapshotEvent? cpu, bool fullAssembly = false)
    {
        ...
        var cpuEip = cpu?.Eip ?? HexUtil.Zero;
        ...
        builder.AppendLine($"CPU真实RIP：{(cpu is null ? "暂无有效CPU现场" : cpuEip)}");
        ...
        var assembly = fullAssembly
            ? selection.BrowseDisassemblyContext
            : SliceContext(selection.BrowseDisassemblyContext, selection.SelectedAddress, 12, 20);
        foreach(var instruction in assembly)
    ...
    public static string FormatComparison(SnapshotEvent? current, SnapshotEvent? previous)
    {
        ...
        var changed = 0;
        foreach(var name in Registers.Append("RFLAGS"))
        {
        ...
        var beforeFlags = DecodeFlags(Register(previous, "RFLAGS"));
        var afterFlags = DecodeFlags(Register(current, "RFLAGS"));
        ...
        XrefContextEvent? context)
    {
        ...
        var builder = new StringBuilder();
        builder.AppendLine("===== x64dbg Context =====").AppendLine();
        ...
        builder.AppendLine($"真实RIP：{cpu?.Eip ?? "暂无有效CPU现场"}");
    ...
    {
        details = new CallDetails(HexUtil.Zero, HexUtil.Zero, 0, HexUtil.Zero);
        if(instruction is null)
            return false;
        var match = DirectCallRegex().Match(instruction.Instruction);
        var hasTarget = HexUtil.TryParse(instruction.CallTarget, out var target) && target != 0;
        if(!hasTarget && match.Success)
            hasTarget = HexUtil.TryParse(match.Groups[1].Value, out target) && target != 0;
        if(!hasTarget || !HexUtil.TryParse(instruction.Address, out var address) || instruction.Size <= 0)
            return false;
        details = new CallDetails(
            address.ToString("X16", CultureInfo.InvariantCulture),
            target.ToString("X16", CultureInfo.InvariantCulture),
            instruction.Size,
            unchecked(address + (ulong)instruction.Size).ToString("X16", CultureInfo.InvariantCulture));
        return true;
    }
    ...
    {
        if(name == "RIP" && snapshot.Registers.TryGetValue(name, out var valueEip))
            return valueEip;
        if(name == "RIP")
            return snapshot.Eip;
        return snapshot.Registers.TryGetValue(name, out var value) ? value : HexUtil.Zero;
    }
    ...
    [GeneratedRegex(@"^\s*call\s+(?:0x)?([0-9A-Fa-f]{1,16})\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
